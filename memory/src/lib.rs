...
pub unsafe fn allocate_virtual_memory(h_process: HANDLE, base_address: usize, mut size: usize) -> Option<i32>{
    ...
    dinvoke::dynamic_invoke!(ntdll, "NtAllocateVirtualMemory", func_ptr, ret, h_process, &mut base_address, 0, &mut size, 0x1000 | 0x2000, 0x20);
