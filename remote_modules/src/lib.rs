...
pub unsafe fn get_remote_proc_address(target_process_handle: HANDLE, hmodule: HMODULE, proc_name: String, ordinal: usize, use_ordinal: bool) -> Result<usize, ()>{
    ...
    while i < export_table.NumberOfNames as usize {
        let function_name = read_export_name(target_process_handle, remote_module_base_va, export_name_table[i]);
        if function_name.eq(&proc_name){
            ...
            (export_function_table[export_ordinal_table[i] as usize] <= (export_directory.VirtualAddress + export_directory.Size) ){
                let forward_string: String = read_forward_name(target_process_handle, remote_module_base_va, export_function_table[export_ordinal_table[i] as usize]);
    ...
    // WE are using ordinals
        ...
        if (export_function_table[function_table_index] >= export_directory.VirtualAddress) && 
            (export_function_table[function_table_index] <= (export_directory.VirtualAddress + export_directory.Size)){
                let forward_string: String = read_forward_name(target_process_handle, remote_module_base_va, export_function_table[function_table_index]);
                println!("forward string: {}", forward_string);
                let split: Vec<String> = forward_string.split(".").map(|s| s.to_string()).collect();
                let real_module = split.get(0).unwrap();
                let real_export = split.get(1).unwrap();
...
unsafe fn read_export_name(target_process_handle: HANDLE, remote_module_base_va: usize, export_name: u32) -> String{
...
unsafe fn read_forward_name(target_process_handle: HANDLE, remote_module_base_va: usize, export_function: u32) -> String{
    
    let mut forward_name = String::from("");
    let mut done = false;
    let mut j = 0;
    let temp_char = 0 as u8;
    let temp_char_ptr = std::mem::transmute::<&u8, *mut c_void>(&temp_char);
    ...
    while !done {  
        ReadProcessMemory(target_process_handle, (remote_module_base_va as usize + export_function as usize + j) as *mut c_void, temp_char_ptr, std::mem::size_of::<u8>(), Some(null_mut()));
        if temp_char as char != '\0'{
            forward_name.push(temp_char as char);
        }else {
            done = true;
        }  
        j = j + 1;
